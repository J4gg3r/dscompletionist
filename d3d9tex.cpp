...
HRESULT APIENTRY hkIDirect3DTexture9::UnlockRect(UINT Level)
{
	...
	{
		m_bLocked = false;
		DWORD *pData = (DWORD*)m_pLockedRect->pBits;

		switch(m_Format)
		{
			case D3DFMT_A8R8G8B8:
			{
				m_crc32 = QuickChecksum(pData, 4*m_Width*m_Height);
				break;
			}
			case D3DFMT_FORCE_DWORD:
			{
				m_crc32 = QuickChecksum(pData, 1*m_Width*m_Height);
				break;
			}
			default:
			{
				m_crc32 = QuickChecksum(pData, (m_Width*m_Height)/2);
				break;
			}
		}    
	}
