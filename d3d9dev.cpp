...
void hkIDirect3DDevice9::InitializeRadar()
{
	m_pFont = D3DUtils::InitializeFont(m_pD3Ddev);
	m_pLine = D3DUtils::InitializeLine(m_pD3Ddev);
	D3DUtils::GetWindowResolution(&m_PresentParam, &m_width, &m_height);
}

void hkIDirect3DDevice9::ReleaseRadar()
{
	D3DUtils::ReleaseFont(m_pFont);
	D3DUtils::ReleaseLine(m_pLine);
	m_pFont = NULL;
	m_pLine = NULL;
}
...
HRESULT APIENTRY hkIDirect3DDevice9::EndScene()
{
	if (m_pFont)
	{
		radar.Draw(m_pD3Ddev, m_pFont, m_pLine, m_width, m_height);
	}
